...
from .utils import (
    ...
    allocate_rust_names,
    generated_rust_type_path,
    is_type_erased,
    render_rust_type,
    split_rust_module_prefix,
    split_rust_type_key,
)
...
class _ObjectRenderer:
    ...
    def _ty_render(self, origin: str) -> str:
        ...
        if mapped is None:
            ...
            if self.imports.has_import(origin):
                mapped = origin
            else:
                if origin not in self.imports.known_type_keys:
                    raise UnsupportedTypeError(
                        origin,
                        "referenced object is not present in the generation inputs; "
                        "generate its marker/prefix or add an explicit ty-map/import",
                    )
                return self._generated_type_path(origin)
        return self.imports.record(mapped)

    def _generated_type_path(self, type_key: str, suffix: str = "") -> str:
        ...
        *this* file's module is a local item: bare leaf, no ``use``. A head
        ...
        """
        return self.imports.generated_type_path(type_key, suffix)

    def _is_object_ref(self, origin: str) -> bool:
        """Return whether the selected Rust carrier is proven ObjectRefCore."""
        mapped = self.ty_map.get(origin)
        if mapped is not None:
            return (
                origin in C_RUST.RUST_OBJECT_REF_ORIGINS
                and mapped == C_RUST.RUST_TY_MAP_DEFAULTS.get(origin)
            )
        return origin in self.imports.known_type_keys or self.imports.has_import(origin)
    ...
    def render_struct_field(self, schema: NamedTypeSchema) -> str:
        ...
        return render_rust_type(schema, self._ty_render, self._is_object_ref)
    ...
    def _resolve_parent(self) -> None:
        ...
        if parent_key not in self.imports.known_type_keys:
            raise UnsupportedTypeError(
                parent_key,
                "generated child requires its parent object marker in the generation inputs",
            )
        self.parent_ref = self._generated_type_path(parent_key)
        self.base_type = self._generated_type_path(parent_key, "Obj")
    ...
    def _explicit_init_methods(self) -> list[FuncInfo]:
        ...
        for method in methods:
            ...
            while result.origin == "Optional":
                (result,) = result.args
            # Native `refl::init<...>` factories canonically return ObjectRef,
            # whose normalized reflection schema is `Object`.  Some producers
            # preserve the concrete type instead.  In either case the packed
            # result is strictly converted to `Leaf` before this safe wrapper
            # can return it.
            if result.origin not in {"Object", self.info.type_key}:
                raise UnsupportedTypeError(
                    result.origin,
    ...
    def _new_fn_explicit(self, method: FuncInfo, name: str = "ffi_new") -> list[str]:
        ...
        params = [
            (
                f"_{index}",
                self.render_param(schema),
                schema.origin == "Any" or is_type_erased(schema),
            )
            for index, schema in enumerate(rest)
        ]
        self.imports.record("tvm_ffi::Result")
        if params:
            self.imports.record("tvm_ffi::AnyView")
        packed = _packed_args_expr(params, False)
        signature = ", ".join(f"{param}: {ty}" for param, ty, _ in params)
        getter = self._cached_getter_lines("f", "__ffi_init__")
        return [
            f"pub fn {_escape_ident(name)}({signature}) -> Result<{self.leaf}> {{",
            *_packed_call_lines("f", getter, packed, self.leaf, returns_any=False),
            "}",
        ]
    ...
    def _method_fn(self, method: FuncInfo, rust_name: str) -> list[str]:
        ...
        ret = (
            ...
            else render_rust_type(ret_schema, self._ty_render, self._is_object_ref)
        )
        rest = args[2:] if method.is_member else args[1:]
        params = [
            (
                f"_{i}",
                self.render_param(p),
                p.origin == "Any" or is_type_erased(p),
            )
            for i, p in enumerate(rest)
        ]

        if method.is_member:
            sig_parts = ["&self", *[f"{n}: {t}" for n, t, _ in params]]
        else:
            sig_parts = [f"{n}: {t}" for n, t, _ in params]
        self.imports.record("tvm_ffi::Result")
        ...
        return [
            ...
            *_packed_call_lines(
                ...
                packed,
                ret,
                returns_any=ret_schema.origin == "Any" or is_type_erased(ret_schema),
            ),
            "}",
        ]
...
def generate_rust_object(
    ...
    safe reflection-backed field getters, reflected methods, and explicit
    native constructors. Generic field-wise initialization is intentionally
    omitted because reflection does not prove constructor invariants.
    ...
    leaf = generated_rust_type_path(type_key).rsplit("::", 1)[-1]
    obj_struct = generated_rust_type_path(type_key, "Obj").rsplit("::", 1)[-1]
    imports.reserve_local(leaf, obj_struct)
...
def _render_rust_global_func(
    ...
    is_object_ref: Callable[[str], bool],
    imports: RustImports,
) -> list[str]:
    ...
    ret = (
        ...
        else render_rust_type(ret_schema, ty_render, is_object_ref)
    )
    params: list[tuple[str, str, bool]] = []
    for index, schema in enumerate(schema_args[1:]):
        erased = schema.origin == "Any" or is_type_erased(schema)
        ty = (
            f"{imports.record('tvm_ffi::AnyView')}<'_>"
            if erased
            else render_rust_type(schema, ty_render, is_object_ref)
        )
        params.append((f"_{index}", ty, erased))
    if params:
        imports.record("tvm_ffi::AnyView")
...
def generate_rust_global_funcs(
    ...
    def generated_path(type_key: str) -> str:
        return imports.generated_type_path(type_key)
    ...
    def ty_render(origin: str) -> str:
        ...
        if mapped is None:
            if "." not in origin or origin.startswith("ctypes."):
                raise UnsupportedTypeError(origin)
            if imports.has_import(origin):
                mapped = origin
            else:
                if origin not in imports.known_type_keys:
                    raise UnsupportedTypeError(
                        origin,
                        "referenced object is not present in the generation inputs; "
                        "generate its marker/prefix or add an explicit ty-map/import",
                    )
                return generated_path(origin)
        return imports.record(mapped)

    def is_object_ref(origin: str) -> bool:
        mapped = ty_map.get(origin)
        if mapped is not None:
            return (
                origin in C_RUST.RUST_OBJECT_REF_ORIGINS
        ...
        return origin in imports.known_type_keys or imports.has_import(origin)
    ...
    for func, rust_name in zip(global_funcs, rust_names):
        section = _render_rust_global_func(func, rust_name, ty_render, is_object_ref, imports)
        if rendered:
            rendered.append("")
        rendered += section
...
def generate_rust_api_file(
    ...
    if not code_blocks:
        ...
        # Opaque marker/handle fields are consumed by derives and the FFI ABI,
        # not by ordinary Rust field reads. Reflected names retain their source
        # spelling, including non-idiomatic casing.
...
def _remove_managed_module_block(mod_rs: Path, lines: list[str]) -> list[str]:
    """Validate and remove one previously managed module block."""
...
def plan_rust_module_tree(
    init_path: Path,
    prefixes: set[str],
    overlay: Mapping[Path, str] | None = None,
    generated_items: Mapping[Path, set[str]] | None = None,
) -> dict[Path, str]:
    """Plan exact module wiring for the prefixes in this invocation."""
    staged = {path.resolve(): source for path, source in (overlay or {}).items()}
    item_names = {
        path.resolve(): {name.removeprefix("r#") for name in names}
        for path, names in (generated_items or {}).items()
    }

    desired_parts = [split_rust_module_prefix(prefix) for prefix in sorted(prefixes)]
    children: dict[tuple[str, ...], set[str]] = {}
    for segs in desired_parts:
        for i, seg in enumerate(segs):
            children.setdefault(segs[:i], set()).add(seg)

    planned: dict[Path, str] = {}
    for parts in sorted(children):
        parent = init_path.joinpath(*parts)
        names = children[parts]
        mod_rs = parent / "mod.rs"
        existing = staged.get(
            mod_rs.resolve(), mod_rs.read_text(encoding="utf-8") if mod_rs.exists() else ""
        )
        lines = existing.splitlines()
        output = _remove_managed_module_block(mod_rs, lines)
        ...
        conflicts = item_names.get(mod_rs.resolve(), set()).intersection(names | external_names)
        if conflicts:
            joined = ", ".join(sorted(conflicts))
            raise UnsupportedTypeError(
                joined,
                f"Rust module/type namespace collision in {mod_rs}: {joined}",
            )
        managed_names = names - external_names
        ...
        output += [
            "",
            _MODULES_BEGIN,
            *[f"pub mod {_escape_ident(name)};" for name in sorted(managed_names)],
            _MODULES_END,
        ]
        planned[mod_rs] = "\n".join(output) + "\n"
    return planned
...
def finalize_rust_module_tree(init_path: Path, prefixes: set[str]) -> None:
    ...
    Every target is parsed and rendered before the first write. Each changed
    file is then replaced atomically.
    """
    planned = plan_rust_module_tree(init_path, prefixes)
    changes = {
        path: source
