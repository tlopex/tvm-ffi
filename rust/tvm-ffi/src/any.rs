...
///
/// This is public for code generated by TVM-FFI macros.
#[inline]
#[doc(hidden)]
pub fn try_cast_from_any_view<T>(value: &AnyView<'_>) -> Result<T, ()>
where
    T: AnyCompatible,
{
...
#[inline]
pub(crate) fn try_cast_from_any<T>(value: Any) -> Result<T, ()>
where
    T: AnyCompatible,
{
    unsafe {
        if T::check_any_strict(&value.data) {
            ...
            Ok(T::move_from_any_after_check(&mut value.data))
        } else {
